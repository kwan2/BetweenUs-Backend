...
export class KanbanController {
    ...
    @Put(':id')
    async createKanban (@Param('id') space_id : number , @Body() kanbanDto : kanbanRO) : Promise<ResponseDto<kanbanRO>> {
        const kanban = await this.kanbanService.createKanban(kanbanDto.content, kanbanDto.state, space_id);
    ...
    @HttpCode(HttpStatus.CREATED)
    @Get(':id/:state')
    async getKanban (@Param('id') space_id : number , @Param('state') state : number ) : Promise<ResponseDto<kanbanRO[]>> {
