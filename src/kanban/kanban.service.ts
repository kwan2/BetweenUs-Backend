...
export class KanbanService {
    ...
    async createKanban(content : string , state : number , space_id : number) : Promise<any> {
        const kanbanEntity = new KanbanEntity();
        kanbanEntity.content = content;
        kanbanEntity.state = state;
        kanbanEntity.space_id = space_id;

        await this.kanbanRepository.createQueryBuilder()
            .insert()
    ...
    async getKanbanlist (space_id : number, state : number) : Promise<KanbanEntity[]> {
