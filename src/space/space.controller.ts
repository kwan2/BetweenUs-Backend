...
export class SpaceController {
    ...
    @Public()
    @Post()
    async createSpace (@Body() user_id : number , hackathon_id: number ) : Promise <any> {
        const teamid = await this.participantService.getTeamID(user_id,hackathon_id);
        console.log(teamid);
    ...
    @Public()
    @Get(':id/:part')
    async getSpace (@Param('id') space_id : number ,@Param('part') part : string ) : Promise<ResponseDto<SpaceEntity[]>> {
